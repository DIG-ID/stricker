<article id="post-<?php the_ID(); ?>" <?php post_class( 'card-news mb-10 bg-light-blue border-t-[7px] border-blue' ); ?>>
    <a href="<?php the_permalink(); ?>" class="grid grid-cols-4 md:grid-cols-6 xl:grid-cols-3">
        <div class="col-span-1 md:col-span-2 xl:col-span-1 col-start-1 md:col-start-1 xl:col-start-1">
            ...
            ?>
                <div class="featured-image h-full">
                    <img src="<?php echo $full_image_url[0]; ?>" alt="<?php the_title(); ?>" class="object-cover w-full h-full" />
                </div>
            <?php else : ?>
        ...
        <div class="col-span-3 md:col-span-4 xl:col-span-2 col-start-2 md:col-start-3 xl:col-start-2 py-4 min-h-[86px] md:min-h-[112px]">
            <?php $formatted_date = get_the_date('d/m/y');
            echo '<p class="st-body-dates text-blue px-5">' . $formatted_date . '</p>';
            ?>
            <h2 class="st-subtitle-i !leading-normal !text-[14px] md:!text-[17px] !font-[545] text-dark-blue px-5"><?php the_title(); ?></h2>
        </div>
    </a>
</article>